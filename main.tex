...
%
This will ensure that all goal states are covered by the all replanable states. Namely, it will allow to immediately start executing a new path once the goal location is updated by~$\calP$.
%
...
The primitive is computed from a state $s \in \Pi_i$ to $s' \in \Pi_j$ such that $t(s') = t(s) + \delta t$ by simple interpolation while ensuring that kinodynamic constraints of the robot are satisfied. Specifically, given the nominal joint velocities of the robot if $s'$ can be reached in time $\delta t$, while respecting the kinematic and collision constraints, then the transition is allowed.
...
In order to pick a known object $o$ moving object along the conveyor~$\calB$, we need a method to estimate its 3-DoF pose at various locations across~$\calB$. Thus, we use an ICP-based pose estimation strategy that uses the known 3D model of~$o$ and the initial pose estimate obtained after pre-processing the input point cloud. The pre-processing step involves removal of all points that lie outside $\calB$, yielding only points corresponding to the object of interest. 
The mean of the remaining points is then used as an initial translation estimate for ICP. 
The rotation estimate is applied in a way that it places the object parallel to its direction of motion along the conveyor. 
The object's 3D model is transformed with this initial estimate and ICP refinement is performed between the resulting point cloud and the pre-processed observed point cloud. 
% The resulting transform is concatenated with the initial estimate to return the final predicted pose. 
% For speeding up ICP, we downsample all point clouds to a leaf size of 15mm.
%
