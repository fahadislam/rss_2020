...
\begin{figure}[t]
    ...
    (\subref{fig:naive1})~At $t = 0$, the algorithm computes $n_{\rm goal}$ paths, that is from \Shome to every $g \in \Gfull$.
    (\subref{fig:naive2})~At $t = \delta_t$, the algorithm computes $n_{\rm goal}^2$ paths, that is from all $n_{\rm goal}$ replanable states at $t = \delta_t$ to every $g \in \Gfull$ (here we only show paths from three states).
