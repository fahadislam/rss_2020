...
\begin{figure*}[t]
    ...
    %
    (\subref{fig:pl3})~We then try to compute new root path with it's corresponding goal states (see also Fig.~\ref{fig:crp2}).
    %
...
\begin{algorithmic}[1]
    \If{$s_{\textrm{start}} = \Shome$}
    \Comment{First estimeate from $\calP$}
...
\os{Oren - Do you think we need a correctness proof for the whole algorithm?}
